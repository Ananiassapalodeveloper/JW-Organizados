...
import { RemoverDesignacaoDialogTsouros } from "@/components/ReuniaoMeioDeSemanaRemover/RemoverTesouro/removedor-designacao-dialog";
...
export function Tesouros({ params }: { params: { id: string|undefined } }) {
  ...
  // Form setup
  const methods = useForm<TesourosType>({
  ...
  const QTDSMembro = useMemo(() => {
    return memberDirigente?.Tesouros.filter(
      (ts) => ts.lesson === lesson && ts.memberId === memberDirigenteId
    )?.length;
  }, [lesson, memberDirigente?.Tesouros, memberDirigenteId]);

  const QTDSuplente = useMemo(() => {
    return memberDirigenteSuplente?.TesourosSuplente.filter(
      (ts) =>
        ts.lesson === lesson &&
        ts.suplenteMemberId === memberDirigenteSuplenteId
    )?.length;
  }, [
    lesson,
    memberDirigenteSuplente?.TesourosSuplente,
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge
              className={cn(
                "rounded-full",
                CompletedDisignation(Tesouros?.length >= 3)
              )}
              variant="secondary"
            >
              {Tesouros?.length || 0} designações
            </Badge>
            <RemoverDesignacaoDialogTsouros id={id} />
          </div>
        </div>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4 bg-card rounded-lg border p-4">
                ...
                <QTDsDesignation
                  isPermitted={lesson !== "" && memberDirigenteId !== ""}
              ...
              <div className="space-y-4 bg-card rounded-lg border p-4">
                ...
                <QTDsDesignation
                  isPermitted={
                    lesson !== "" && memberDirigenteSuplenteId !== ""
                  }
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>
                            Seleciona o ponto para veres quantas vezes o irmão
                            já fez a designação
                          </p>
                        </TooltipContent>
                      </Tooltip>
