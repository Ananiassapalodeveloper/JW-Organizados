...
export function PresidentEndPraying({ params }: { params: { id: string|undefined } }) {
  ...
  const { data: existingDesignation, isLoading: isLoadingDesignation } =
    useFetch<PresidenteType>(`partesInicias/${params.id}`);
  ...
  async function onSubmit(values: PartesIniciasDono) {
    try {
      ...
      if (existingDesignation) {
        ...
        await api.patch(`partesInicias`, formData);
        toast({
        ...
        await api.post("partesInicias", formData);
        toast({
        ...
        <Form {...form}>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-medium">Presidente</h3>
