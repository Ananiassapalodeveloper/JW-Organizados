...
import { RemoverDesignacaoDialog } from "@/components/ReuniaoFimDeSemanaRemovedor/RemoverReuniaoPublica/removedor-designacao-dialog";
import {
...
export function ReuniaoPublica({ params }: { params: { id: string|undefined } }) {
  ...
  const {
    data: reuniaoPublica,
    error: reuniaoPublicaError,
    isLoading: reuniaoPublicaLoading,
    mutate,
  } = useFetch<any>(`reuniaoPublica/${params.id}`);

  // Form setup
  const methods = useForm<reuniaoPublicaType>({
  ...
  const onSubmit = useCallback(
    async (values: reuniaoPublicaType) => {
      try {
        setSubmitError(null);
        ...
        mutate(`reuniaoPublica/${params.id}`);
        BroMutate();
      } catch (error) {
  ...
  if (isLoading || reuniaoPublicaLoading) {
    return (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge
              className={cn(
                "rounded-full",
                CompletedDisignation(reuniaoPublica?.length >= 3)
              )}
              variant="secondary"
            >
              {reuniaoPublica?.length || 0} designações
            </Badge>
