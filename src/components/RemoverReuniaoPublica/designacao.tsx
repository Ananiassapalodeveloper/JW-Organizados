...
import { ReuniaoPublicaValueType } from "@/app/main/(dashboard)/components/CardsDesignations/WatchTowerSpeeching/ReuniaoPublica"
import { RemoverDesignacaoSkeleton } from "./RemovedorDesignacaoSkeleton"
import { mutate } from "swr"

type RegistoReuniaoPublica = {
  id: string
  name: string
  membro: {
    nome: string
...
export function RemoverDesignacao({ inDialog = false, onComplete, id }: RemoverDesignacaoProps) {
  ...
  const { data, error, isLoading } = useFetch<RegistoReuniaoPublica[]>(`reuniaoPublica/${id}`)
  ...
  return (
    <div className={inDialog ? "" : "border rounded-lg shadow-sm"}>
      ...
      <div className={`grid gap-3 ${inDialog ? "" : "p-4"}`}>
        {data && data.length > 0 ? (
          data.map((des) => {
            const cristaoInfo = ReuniaoPublicaValueType.find((m) => m.value === des.name)
