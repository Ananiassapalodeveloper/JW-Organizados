...
export function RemoverDesignacaoDialog({ id }: { id: string | null }) {
  ...
  const handleComplete = () => {
    ...
    const designationTypes = ["presidente", "oracaoInicial", "orador"]
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button title="Remover Designações" size={"icon"} variant="destructive" className="flex items-center gap-2">
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Selecione as designações que deseja remover. Esta ação é irreversível.</DialogDescription>
        </DialogHeader>
