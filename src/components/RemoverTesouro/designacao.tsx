...
import { RegistoTesouro } from "@/types/reuniaoMeioSemanaDTO/type";
...
export function RemoverDesignacaoTesouro({
  ...
  const { data, error, isLoading, mutate } = useFetch<RegistoTesouro[]>(
    `tesouros/${id}`
  );
  ...
  return (
    <div className={inDialog ? "" : "border rounded-lg shadow-sm"}>
      ...
      <div className={`grid gap-3 ${inDialog ? "" : "p-4"}`}>
        {data && data.length > 0 ? (
          data.map((des) => {
            const ministerioInfo = NameTesouros.find(
              (m) => m.value === des.name
            );
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="space-y-1">
                        <h4 className="font-medium text-xs text-muted-foreground">
                          {des.name === "leitura" ? "Leitor" : "Orador"}
                        </h4>
                        <p className="font-medium">{des.membro.nome}</p>
                        <p className="text-xs text-muted-foreground">
                          {dirigenteEstado?.name || des.membro.estado}
                        </p>
                        {des.name === "leitura" && (<span className="text-muted-foreground">ponto de conselho:{" "}<Badge className="rounded-full" variant={"secondary"}>{des?.lesson }</Badge></span>)}
                      </div>
