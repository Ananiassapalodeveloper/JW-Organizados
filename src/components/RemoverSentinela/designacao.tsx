...
import { sentinelaValueType } from "@/app/main/(dashboard)/components/CardsDesignations/WatchTowerSpeeching/WatchTowerLead"
import { RemoverDesignacaoSkeleton } from "./RemovedorDesignacaoSkeleton"
import { mutate } from "swr"

type RegistoSentinela = {
  id: string
  name: string
  membro: {
    nome: string
...
export function RemoverDesignacao({ inDialog = false, onComplete, id }: RemoverDesignacaoProps) {
  const [submitError, setSubmitError] = useState<string | null>(null)
  const [isDeleting, setIsDeleting] = useState<string | null>(null)
  const [selectedItems, setSelectedItems] = useState<string[]>([])

  const { data, error, isLoading } = useFetch<RegistoSentinela[]>(`sentinela/${id}`)
  ...
  async function handleDelete(id: string) {
    try {
      ...
      const response = await api.delete(`sentinela/${id}`)
      ...
      if (designationType) {
        mutate(`sentinela/${id}/${designationType}`)
      } else {
        // If designation type is not available, refresh all types
        ;["dirigente", "leitor", "oracaoFinal"].forEach((type) => {
      ...
      if (error instanceof AxiosError) {
        const errorMessage = error.response?.data?.message || "Erro ao remover a designação"
  ...
  async function handleBulkDelete() {
    ...
    try {
      ...
      for (const itemId of selectedItems) {
        await api.delete(`sentinela/${itemId}`)
      }
      ...
      if (error instanceof AxiosError) {
        const errorMessage = error.response?.data?.message || "Erro ao remover as designações"
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Não foi possível carregar os dados das designações. Por favor, tente novamente.
        </AlertDescription>
      </Alert>
    )
  }
    ...
    <div className={inDialog ? "" : "border rounded-lg shadow-sm"}>
      {!inDialog && (
        <div className="p-4 bg-muted/40 border-b">
          ...
          <p className="text-sm text-muted-foreground">Esta ação é irreversível. Tem certeza que deseja eliminar?</p>
        </div>
      )}

      {submitError && (
        <Alert variant="destructive" className={inDialog ? "mb-4" : "mx-4 mt-2"}>
          <AlertCircle className="h-4 w-4" />
          <AlertTitle>Erro</AlertTitle>
      ...
      {inDialog && selectedItems.length > 0 && (
        <div className="flex items-center justify-between mb-4 px-1">
          <span className="text-sm font-medium">{selectedItems.length} designação(ões) selecionada(s)</span>
      ...
      <div className={`grid gap-3 ${inDialog ? "" : "p-4"}`}>
        {data && data.length > 0 ? (
          data.map((des) => {
            const cristaoInfo = sentinelaValueType.find((m) => m.value === des.name)
            const dirigenteEstado = Estado.find((e) => e.value === des.membro.estado)
            const moradorEstado = Estado.find((e) => e.value === des.suplenteMembro.estado)

            return (
              <div
                key={des.id}
                className={`rounded-lg border p-3 transition-all ${
                  isSelected(des.id) ? "bg-primary/5 border-primary/30" : "hover:bg-muted/50"
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-2 flex-1">
                    <div className="flex items-center justify-between">
                      <Badge variant="outline" className="bg-primary/10 text-primary font-medium px-2.5 py-0.5">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="space-y-1">
                        <h4 className="font-medium text-xs text-muted-foreground">Dirigente</h4>
                        <p className="font-medium">{des.membro.nome}</p>
                        <p className="text-xs text-muted-foreground">{dirigenteEstado?.name || des.membro.estado}</p>
                      </div>

                      <div className="space-y-1">
                        <h4 className="font-medium text-xs text-muted-foreground">Morador</h4>
                        <p className="font-medium">{des.suplenteMembro.nome}</p>
                        <p className="text-xs text-muted-foreground">
                          {moradorEstado?.name || des.suplenteMembro.estado}
                        </p>
                      </div>
                    </div>
          ...
          <div className="text-center py-6 text-muted-foreground">Nenhuma designação encontrada.</div>
        )}
      </div>
    </div>
  )
