...
export function RemoverDesignacao({
  ...
  const { data, error, isLoading } = useFetch<Registoindicadores[]>(
    `indicadores/${id}`
  );
  ...
  async function handleDelete(id: string) {
    try {
      ...
      mutate(`indicadores/${id}`);

      // Also refresh the specific designation type data if available
      if (designationType) {
        mutate(`indicadores/${id}/${designationType}`);
      } else {
        // If designation type is not available, refresh all types
        ["sectorA", "sectorB", "porta"].forEach((type) => {
          mutate(`indicadores/${id}/${type}`);
        });
      }
  ...
  async function handleBulkDelete() {
    ...
    try {
      ...
      for (const itemId of selectedItems) {
        await api.delete(`indicadores/${itemId}`);
      }
