...
import { RemoverDesignacaoDialog } from "@/components/RemoverCristao/removedor-designacao-dialog";
...
export function CristaoPage({ params }: { params: { id: string } }) {
  ...
  const { data: existingDesignation, isLoading: isLoadingDesignation } =
    useFetch<ReturCristao>(`cristao/${params.id}/${watch("name")}`);
  ...
  useEffect(() => {
    if (existingDesignation) {
      form.reset({
        ...
        leitorId: existingDesignation?.LeitorEstudoBiblico?.id,
        leitorSuplenteId: existingDesignation?.LeitorSuplenteEstudoBiblico?.id,
        memberId: existingDesignation?.membro?.id,
        suplenteMemberId: existingDesignation?.suplenteMembro?.id,
        ReunioesDatesId: params?.id,
        tema: existingDesignation?.tema,
      });
    } else {
      form.reset({
        ...defaultValues,
  ...
  }, [existingDesignation, params.id, form, watch]);
  ...
  async function onSubmit(values: cristaoType) {
    try {
      ...
      if (existingDesignation) {
        ...
        await api.put(`cristao/${existingDesignation.id}`, formData);
        toast({
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        ...
                        defaultValue={
                          existingDesignation?.name || field.value?.toString()
                        }
                      >
